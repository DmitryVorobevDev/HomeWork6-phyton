...
# решение с усложнением 1
def find_func (list1, min, max):
    result_list = [i for i in range(len(list1)) if min <= list1[i] <= max]
    print(result_list)

min, max = map(int, input('Введите значения через пробел:  ').split())
list1 = [-5, 9, 0, 3, -1, -2, 1, 4, -2, 10, 2, 0, -9, 8, 10, -9, 0, -5, -5, 7]
find_func(list1, min, max)

# решение с умложнением 2

def find_func (list1, min, max):
    result_list = [i for i in enumerate(list1) if min <= list1[i[0]] <= max]
    print(result_list)
