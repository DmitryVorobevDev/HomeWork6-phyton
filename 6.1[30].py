...
#
# ```(**)``` **Усложнение.** Присвоение значений переменным a1,d,n
# запишите, используя только один input, в одну строку, вам понадобится распаковка и Comprehension или map

a, d, n = map(int, input('Введите значения через пробел:  ').split())
def create_func (a, d, n):
    print(list(range(a, a + n * d, d)))
create_func(a, d, n)